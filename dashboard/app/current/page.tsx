...
export default function CurrentLessonPage() {
  ...
  return <div className="page">{error&&<ProgressNotice message={error} retry={retry}/>}<header className="page-header"><div><p className="eyebrow">Current lesson · Week {lesson.week}, Day {lesson.day}</p><h1>{lesson.title}</h1><p>{lesson.summary}</p>{stale&&<span className="stale-label">Last verified progress</span>}</div><StatusBadge status="IN PROGRESS"/></header>
