...
export default function OverviewPage() {
  ...
  return <div className="page overview-page">
    {error&&<ProgressNotice message={error} retry={retry}/>}<header className="course-heading"><div><p className="eyebrow">Python + AI Software Engineering</p><h1>Resume at <code>{lesson.checkpoint?.current}</code></h1><p>Week {lesson.week}, Day {lesson.day} · Building toward <strong>Autonomous MarketingOps AI</strong></p>{stale&&<span className="stale-label">Last verified progress</span>}</div><StatusBadge status="IN PROGRESS"/></header>
    ...
    <div className="overview-grid">
      ...
      <aside className="panel next-panel"><p className="kicker">Do this next</p><h2>Continue <code>{lesson.checkpoint?.current ?? lesson.title}</code></h2><p>Finish the bounded retry practice, then move into while-loop state and safe termination.</p><div className="acceptance"><strong>Up next</strong><ul>{lesson.checkpoint?.remainingSteps.slice(0,3).map((item) => <li key={item}>{item}</li>)}</ul></div><LessonLink lesson={lesson} label="Continue current lesson"/></aside>
    </div>
