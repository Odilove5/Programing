...
describe("dashboard views with fictional learner fixtures", () => {
  ...
  it("builds history from stored timestamps only", () => { const events=historyEvents(verifiedState()); expect(events.some(e=>e.type==="LESSON STARTED"&&e.title.includes("Values"))).toBe(true); expect(events.every(e=>Boolean(e.date))).toBe(true); });
});
