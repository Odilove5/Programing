...
def lesson_directory(week: int, day: int, unit: dict[str, str]) -> Path:
    return OUTPUT / f"week-{week:02d}" / f"day-{day:02d}-{lesson_presentation(week, day)['slug']}"
...
def write_manifest() -> None:
    import json
    manifest = canonical_lessons()
    (OUTPUT / "curriculum-manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    dashboard = ROOT / "dashboard" / "lib" / "canonical-manifest.ts"
    dashboard.write_text("// Generated by tools/export_course_lessons.py; do not edit.\nexport const canonicalLessonManifest = " + json.dumps({r['id']: r for r in manifest}, indent=2) + " as const;\n", encoding="utf-8")
